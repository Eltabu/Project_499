
<!-- Section Pricing
================================================== -->
<section id="pricing" class="pricing-padding">
<div class="container">
<div class="row">
	<div class="col-lg-12 text-center">
		<h1 class="section-heading"><b>PRICING</b></h1>
		<hr class="primary">
		<br/>
	</div>
</div>
</div>
<div class="container">
<div class="row">
	<div class="wow-pricing-table col-md-4">
		<div class="wow-pricing">
			<div class="wow-pricing-header">
				<h5>Free Trial</h5>
				<div class="wow-pricing-cost">
					 FREE
				</div>
				<div class="wow-pricing-per">
					 For 30 Days
				</div>
			</div>
			<div class="wow-pricing-content">
				<ul>
					<li>Fully Functional</li>
					<li>Multiple Workstations</li>
					<li>Free Support</li>
					<li>Free Upgrades</li>
					<li>Free Hosting Service</li>
					<li>30-Day Trial</li>
				</ul>
			</div>
			<div class="wow-pricing-button">
				<a href="<?php echo URL ?>TestDrive" class="wow-button buttonprice" target="_self" rel="nofollow"><span class="wow-button-inner">Sign Up</span></a>
			</div>
		</div>
	</div>
	<!-- /.col-md-4 -->
	<div class="wow-pricing-table col-md-4">
		<div class="wow-pricing featured">
			<div class="wow-pricing-header">
				<h5>Standard</h5>
				<div class="wow-pricing-cost">
					 $199.99
				</div>
				<div class="wow-pricing-per">
					 One time
				</div>
			</div>
			<div class="wow-pricing-content">
				<ul>
					<li>Up to 25 vehicles</li>
					<li>Multiple Workstations</li>
					<li>Premium Support</li>
					<li>Free Upgrades</li>
					<li>Free Hosting Service</li>
					<li>$19.99 Mountly</li>
				</ul>
			</div>
			<div class="wow-pricing-button">
				<a href="<?php echo URL ?>SignUp?produnct_id=1" class="wow-button buttonprice" target="_self" rel="nofollow"><span class="wow-button-inner">Sign Up</span></a>
			</div>
		</div>
	</div>
	<!-- /.col-md-4 -->
	<div class="wow-pricing-table col-md-4">
		<div class="wow-pricing">
			<div class="wow-pricing-header">
				<h5>Untimate</h5>
				<div class="wow-pricing-cost">
					 $399.99
				</div>
				<div class="wow-pricing-per">
					 One time
				</div>
			</div>
			<div class="wow-pricing-content">
				<ul>
					<li>Unlimited vehicles</li>
					<li>Multiple Workstations</li>
					<li>Premium Support</li>
					<li>Free Upgrades</li>
					<li>Free Hosting Service</li>
					<li>$29.99 Mountly</li>
				</ul>
			</div>
			<div class="wow-pricing-button">
				<a href="<?php echo URL ?>SignUp?produnct_id=2" class="wow-button buttonprice" target="_self" rel="nofollow"><span class="wow-button-inner">Sign Up</span></a>
			</div>
		</div>
	</div>
	<!-- /.col-md-4 -->
</div>
</div>
</section>
<div class="clearfix">
</div>
